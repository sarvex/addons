...
def _check_tf_version():
    ...
    warnings.warn(
        f"Tensorflow Addons supports using Python ops for all Tensorflow versions above or equal to {INCLUSIVE_MIN_TF_VERSION} and strictly below {EXCLUSIVE_MAX_TF_VERSION} (nightly versions are not supported). \n The versions of TensorFlow you are currently using is {tf.__version__} and is not supported. \nSome things might work, some things might not.\nIf you were to encounter a bug, do not file an issue.\nIf you want to make sure you're using a tested and supported configuration, either change the TensorFlow version or the TensorFlow Addons's version. \nYou can find the compatibility matrix in TensorFlow Addon's readme:\nhttps://github.com/tensorflow/addons",
        UserWarning,
    )
